...
alias config-tmux="nvim ~/.config/tmux"
alias config-nvim="nvim ~/.config/nvim"
alias refresh-zsh=". ~/.zshrc"
alias config-zsh="nvim ~/.zshrc"
alias op="op.exe"
alias clear-git='REMOTE=${1:-origin}; git branch -r | grep -v "\->" | sed "s/ *$REMOTE\///" > /tmp/remote_branches.txt && git branch | grep -v "^\*" | grep -vf /tmp/remote_branches.txt | xargs -n 1 git branch -D'
alias clear-phpstan='pkill -f phpstan'
batlog() {
